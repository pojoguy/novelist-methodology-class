...
**Stochastic** means: given the same prompt, the system considers a **range** of possible outputs, not one guaranteed answer. It picks the next word from a ranked list of likely choices — weighted dice, not a fact lookup.
...
The model does not "retrieve a fact." It **samples** likely continuations. That is why unconstrained *"fix my chapter"* produces generic prose. That is why [**Signs**](04-audit-and-governance.md) exist in craft rubrics — names for patterns that look fine until you compare them to **your** voice.
...
**Want the century-by-century lineage** (telephone traffic, information theory, transformers)? See the **[appendix](#appendix-optional-depth--telephone-switching-and-gaines)** — optional on first read.
...
Some fears had merit (homogenization, over-reliance). None eliminated authorship. What changed was **workflow** — and the authors who learned the workflow early had an advantage.
...
- **Autistic** author using LLM as **vocabulary and continuity prosthetic** — external working memory under load, not voice replacement
